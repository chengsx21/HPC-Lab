...
void Ring_Allreduce(void* sendbuf, void* recvbuf, int n, MPI_Comm comm, int comm_sz, int my_rank)
{
    ...
    int recv_rank = (comm_sz + my_rank - 1) % comm_sz;
    float* recvbuf_ = new float[size_];

    for (int step = 0; step < comm_sz - 1; step++) {
        int send_offset = ((comm_sz + my_rank - step) % comm_sz) * size_;        
        int recv_offset = ((comm_sz + my_rank - step - 1) % comm_sz) * size_;
        MPI_Sendrecv(
            (float*)recvbuf + send_offset, size_, MPI_FLOAT, send_rank, 0,
            recvbuf_, size_, MPI_FLOAT, recv_rank, 0,
            comm, nullptr
        );
        for (int offset = 0; offset < size_; offset++) {
            ((float*)recvbuf)[recv_offset + offset] += recvbuf_[offset];
        }
    }

    for (int step = 0; step < comm_sz - 1; step++) {
        int send_offset = ((comm_sz + my_rank + 1 - step) % comm_sz) * size_;
        int recv_offset = ((comm_sz + my_rank - step) % comm_sz) * size_;
        MPI_Sendrecv(
            (float*)recvbuf + send_offset, size_, MPI_FLOAT, send_rank, 0,
            (float*)recvbuf + recv_offset, size_, MPI_FLOAT, recv_rank, 0,
            comm, nullptr
        );
    }
    delete[] recvbuf_;
